...
require_once 'models/modelLevel.php';
...
<div class="container-fluid mt-5 mb-5 allpage">
  <?php if (count($_POST) == 0 || count($formErrors) > 0) { ?>
    <form action="signin.php" method="POST">
      <fieldset>
        <legend class="text-center text-muted"><p class="bienV">Dancers <br> Get in the Tribe !<br>Inscrivez-vous</p></legend>
...
<!--          <div class="form-group row">
            <div class="col-lg-4 col-md-4 col-12">
              <label class="text-muted" for="danceLevel">Niveau de danse</label>
              <select class="form-control text-muted" id="country" name="danceLevel">
                <option selected disabled>---choisissez votre niveau---</option>
                <option value="Débutant"  <?= isset($_POST['danceLevel']) && $_POST['danceLevel'] == 'Débutant' ? 'selected' : '' ?>>Débutant</option>
                <option value="Évolutif"  <?= isset($_POST['danceLevel']) && $_POST['danceLevel'] == 'Évolutif' ? 'selected' : '' ?>>Évolutif</option>
                <option value="Intermédiaire"  <?= isset($_POST['danceLevel']) && $_POST['danceLevel'] == 'Intermédiaire' ? 'selected' : '' ?>>Intermédiaire</option>
                <option value="Avancé"  <?= isset($_POST['danceLevel']) && $_POST['danceLevel'] == 'Avancé' ? 'selected' : '' ?>>Avancé</option>
              </select>
            </div>
          </div>-->
...
<!--après le mot de passe select pour le niveau des danseurs comme dans la vue des artistes-->
<hr class="separateartistoption">

           <div class="col-lg-4 col-md-4 col-12">
            <label class="text-muted" for="level">choisissez un niveau </label>
            <select class="form-control text-muted" id="level" name="level">
              <option selected disabled>---Veuillez choisir votre niveau---</option>
              <?php foreach ($listlevel as $level){ ?>
              <option value="<?php echo $level->id ?>"> <?php echo $level->level ?></option>
              <?php } ?>

            </select>
              <?php if (isset($formErrors['level'])) {
                ?>
                <div class="invalid-feedback"><?= $formErrors['level'] ?></div>
              <?php }
              ?>
            </div>
    ...
    <div class="card border-secondary mb-3">
      ...
      <div class="card-body">
        <p class="card-text">
          ...
          <span class="font-weight-bold">ville :</span> <?= $users->city ?><br />
